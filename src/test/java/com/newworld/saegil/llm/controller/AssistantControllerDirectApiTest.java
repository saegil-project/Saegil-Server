...
//                "남조선 말은 너무 빨라서 알아듣기 힘들어요. 전 표준말이랑 좀 다른데, 괜찮습네까?",
    ...
    void testFastApiAssistantDirectly(String question) {
        ...
        ProxyAssistantRequest requestBody = new ProxyAssistantRequest(question);
        HttpHeaders headers = new HttpHeaders();
        headers.setContentType(MediaType.APPLICATION_JSON);
        HttpEntity<ProxyAssistantRequest> requestEntity = new HttpEntity<>(requestBody, headers);
        ...
        ResponseEntity<ProxyAssistantResponse> responseEntity = restTemplate.exchange(
                requestUrl,
                HttpMethod.POST,
                requestEntity,
                ProxyAssistantResponse.class // Use the ProxyAssistantResponse record
        );

        // Then: Verify the response status and print the response text
        assertThat(responseEntity.getStatusCode()).isEqualTo(HttpStatus.OK);
        ProxyAssistantResponse responseBody = responseEntity.getBody();
        ...
        ProxyAssistantRequest followUpRequestBody = new ProxyAssistantRequest(followUpQuestion);
        HttpEntity<ProxyAssistantRequest> followUpRequestEntity = new HttpEntity<>(followUpRequestBody, headers);
        ...
        ResponseEntity<ProxyAssistantResponse> followUpResponseEntity = restTemplate.exchange(
                followUpUrl,
                HttpMethod.POST,
        ...
        ProxyAssistantResponse followUpResponseBody = followUpResponseEntity.getBody();
